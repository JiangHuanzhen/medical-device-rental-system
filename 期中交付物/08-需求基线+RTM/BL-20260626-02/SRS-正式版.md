...
| V2.0.0 | 2026-06-26 | 俞儿游 | 变更 | 基于校准有效期管理需求重构，新增FR-EQP-004~007，替换原BL-01的FR-EQP-001~003（RTM含FR-EQP-004~009，其中FR-EQP-008/009为隐含需求） | 俞儿游 |
...
2.  **归还检测不规范**：设备归还时的检测流程不统一，依赖个人经验，缺乏与出厂基准数据的自动比对，导致设备潜在故障无法在验收窗口期内被有效识别，影响后续租赁质量。
...
flowchart TD
    ...
    subgraph 接入层[接入层]
        direction LR
        B1[反向代理<br/>负载均衡]
        B2[API网关<br/>路由/鉴权/限流]
        B3[认证中心<br/>身份认证/会话管理]
    end
...
| 需求编号 | 需求描述 | 指标 |
| ---- | ---- | ---- |
| **NFR-PERF-001** | 页面加载时间 | 90%的页面加载时间不超过3秒。 |
| **NFR-PERF-002** | 接口响应时间 | 90%的API接口响应时间不超过500毫秒。 |
| **NFR-PERF-003** | 并发用户数 | 系统应支持至少200个用户同时在线操作。 |
| **NFR-PERF-004** | 吞吐量 | 系统应支持至少1000 TPS（事务/秒）的核心业务处理能力。 |
| **NFR-PERF-005** | 定时任务执行时间 | 每日凌晨的校准预警扫描任务，必须在30分钟内完成对所有设备的扫描。 |

### 3.3.2 可靠性需求

| 需求编号 | 需求描述 | 指标 |
| ---- | ---- | ---- |
| **NFR-REL-001** | 系统可用率 | 系统在7x24小时运行模式下，年度可用率不低于99.9%（即年度计划外停机时间不超过8.76小时）。 |
| **NFR-REL-002** | 连续运行能力 | 系统应能连续运行7天而无需重启，期间无内存泄漏或性能明显下降。 |
| **NFR-REL-003** | 故障恢复时间 | 发生非硬件故障时，系统应在30分钟内恢复服务。 |

### 3.3.3 安全性需求

| 需求编号 | 需求描述 | 指标 |
| ---- | ---- | ---- |
| **NFR-SEC-001** | 用户认证 | 所有用户必须通过用户名+密码+验证码的方式进行身份认证。 |
| **NFR-SEC-002** | 权限控制 | 系统必须实现基于角色的访问控制（RBAC），确保用户只能访问其权限范围内的功能和数据。 |
| **NFR-SEC-003** | 数据加密 | 用户密码必须使用bcrypt算法加盐哈希存储。所有敏感数据（如客户联系方式、合同金额）在传输过程中必须使用HTTPS加密。 |
| **NFR-SEC-004** | 攻击防护 | 系统必须能防御常见的Web攻击，如SQL注入、XSS跨站脚本攻击、CSRF跨站请求伪造。 |
| **NFR-SEC-005** | 操作审计 | 所有用户的关键操作（如登录、创建订单、修改设备状态、审批）必须记录审计日志，日志保留至少180天。 |
...
| **NFR-EXT-002** | 功能扩展 | 系统架构应支持未来新增业务模块（如供应商管理、配件管理）的低耦合集成，新增模块不应影响现有功能运行。 |
...
| CR-202606-001 | 2026-06-26 | 俞儿游 | AI | 基于校准有效期管理需求重构：新增FR-EQP-004~007，替换原BL-01的FR-EQP-001~003；扩展AUTH/CUS/ORD/FIN/STA/CFG模块FR条目 | EQP/AUTH/CUS/ORD/FIN/STA/CFG | 通过 | BL-20260626-02 |
...
| 需求编号 | 需求名称 | 验收标准 | 优先级 |
| ---- | ---- | ---- | ---- |
| FR-EQP-004 | 监控设备校准有效期 | 1. 校准有效期剩余5天的设备，状态变为“预警中”，禁止出库。2. 校准有效期剩余2天的设备，状态变为“锁库中”，禁止出库。 | P0 |
| FR-EQP-007 | 执行设备归还检测 | 1. 偏差超过干预阈值时，自动终结检测并创建P1级工单。2. 偏差在预警与干预阈值之间时，提供“强制入库”和“创建工单”选项。 | P0 |
| FR-EQP-006 | 发起紧急出库审批 | 1. 三级审批全部通过后，锁库设备可出库。2. 任何一级审批拒绝，流程终止。 | P1 |
